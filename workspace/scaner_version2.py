...
class ScanContext:
    ...
    def ptcloud2sc_vectorized(self, ptcloud, num_sector, num_ring, max_length):
        ...
        # Get coordinates and add height offset
        x = ptcloud[:, 0]
        y = ptcloud[:, 1]
        z = ptcloud[:, 2] + ScanContext.kitti_lidar_height
        ...
        # Initialize scan context matrix
        sc = np.zeros((num_ring, num_sector))
        
        # Use numpy's advanced indexing to find max height per cell
        # This is much faster than the for loop approach
        for i in range(len(r)):
            # Only update if this point has a greater height
...
if __name__ == "__main__":
    ...
    # process_dataset_to_images(metadata, output_dir='scan_dl_images_second', image_size=(80, 360))
